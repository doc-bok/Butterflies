...
class DataGenerator:
    ...
    def generate_file_list(self, folder: Union[Path, str]) -> List[str]:
        """
        Generates a list of JSON file names (without extension) in a folder (non-recursive).
        :param folder: Folder path to search.
        :return: List of file stems.
        """
        folder_path = Path(folder)
        self.logger.info(f"Generating file list for folder [{folder_path}]")

        if not folder_path.exists():
            self.logger.warning(f"Folder {folder_path} does not exist.")
            return []

        files = [f.stem for f in folder_path.glob("*.json") if f.is_file()]
        self.logger.debug(f"Generated file list: {files!r}")
        return files

    def generate_item_models(self) -> None:
        """
        Generates missing item model JSON files by clearing old ones and creating new ones
        based on model templates.
        """
        self.logger.info("Resetting item models...")
        try:
            for file in self.butterfly_items_path.iterdir():
                if file.is_file():
                    file.unlink()
        except OSError as e:
            self.logger.error(f"Failed to clear item models in {self.butterfly_items_path}: {e}")

        files = self.generate_file_list(self.butterfly_item_models_path)
        for file in files:
            if "template" not in file:
                output_path = self.butterfly_items_path / f"{file}.json"
                try:
  ...
  "model": {{
    ...
    "type": "minecraft:model"
                    ...
                    self.logger.error(f"Failed to write item model JSON to {output_path}: {e}")